...
class HelpModal {
    ...
    updateModalContent(helpType, helpText) {
        ...
        const titles = {
            ...
            'PROJECT': 'PROJECT Laukas', 
            'TAGS': 'TAGS Laukas',
            'PRIORITY': 'PRIORITY Laukas',
            'ESTIMATE': 'ESTIMATE Laukas'
        };
    ...
    formatHelpText(text) {
        ...
        sections.forEach((section, index) => {
            ...
            // Patikrinti ar sekcija yra hierarchija aprašymai
            else if (section.includes('PROJEKTŲ APRAŠYMAI:')) {
                formattedHTML += this.formatProjectDescriptions(section);
            }
            // Patikrinti ar sekcija yra kategorijos aprašymas
            else if (section.includes('Kategorijos:')) {
                formattedHTML += this.formatCategoriesSection(section);
            }
    ...
    formatProjectDescriptions(section) {
        const lines = section.split('\n');
        let html = '<h4>Projektų aprašymai</h4>';
        html += '<div class="help-info">';
        
        lines.slice(1).forEach(line => {
            line = line.trim();
            if (line.startsWith('-') && line.includes(':')) {
                // Formatuoti kaip project : description
                const cleaned = line.substring(1).trim();
                const [project, ...descParts] = cleaned.split(':');
                const description = descParts.join(':').trim();
                html += `<div><strong>${project.trim()}:</strong> ${description}</div>`;
            } else if (line) {
                html += `<div>${line}</div>`;
            }
        });
    ...
    formatCategoriesSection(section) {
        const lines = section.split('\n');
        const title = lines[0];
        ...
        lines.slice(1).forEach(line => {
            line = line.trim();
            if (line.startsWith('-') && line.includes(':')) {
                // Formatuoti kaip CATEGORY: description (tags)
                const cleaned = line.substring(1).trim();
                const parts = cleaned.split(':');
                if (parts.length >= 2) {
                    const category = parts[0].trim();
                    const description = parts.slice(1).join(':').trim();
                    html += `<div><strong>${category}:</strong> ${description}</div>`;
                }
            } else if (line) {
                html += `<div>${line}</div>`;
            }
