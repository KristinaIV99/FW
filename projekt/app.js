...
class TaskWarriorComposer {
    ...
    exportTasks() {
        ...
        if (!taskCommand || taskCommand === 'task add') {
            this.showFeedback('Nėra duomenų eksportavimui!', 'error');
            return;
        }
        
        // Sukurti tekstinį failą su TaskWarrior komanda
        const blob = new Blob([taskCommand], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        
        const a = document.createElement('a');
        a.href = url;
        a.download = `taskwarrior_command_${new Date().toISOString().split('T')[0]}.txt`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        
        this.showFeedback('TaskWarrior komanda eksportuota!', 'success');
    }
