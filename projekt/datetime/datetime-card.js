...
class DateTimeCard {
    constructor() {
        ...
        this.fieldInfo = {
            due: { title: 'DUE', description: 'Kada turi būti baigta užduotis' },
            wait: { title: 'WAIT', description: 'Iki kada užduotis yra paslėpta' },
            scheduled: { title: 'SCHEDULED', description: 'Kada pradėti arba kada rodyti' },
            recur: { title: 'RECUR', description: 'Užduoties pasikartojimo dažnumas' },
            until: { title: 'UNTIL', description: 'Kada sustabdyti kartojamąją užduotį' }
        };
    }
            ...
            <div class="accordion-item" data-field="${field}">
                <div class="accordion-header" onclick="datetimeCard.toggleAccordion('${field}')">
                    <div class="accordion-title">
                        <span class="field-name">${info.title}</span>
                        <span class="field-description">${info.description}</span>
                    </div>
                ...
                <div class="accordion-content collapsed" id="${field}-accordion-content">
                    <div class="field-help-section">
                        <div class="help-text">${this.getFieldHelpText(field)}</div>
                    </div>
                        ...
                        <div class="component-grid">
                            ${this.fieldConfigs[field].map(component => `
                                <div class="component-card" 
                                    ...
                                    <div class="component-status clickable-component-status" id="${field}-${component}-status" onclick="datetimeCard.clearComponent('${field}', '${component}')" title="Paspausti norėdami išvalyti ${component}">—</div>
                                </div>
                            `).join('')}
                        </div>
            ...
            case 'time':
                return `
                    <div class="time-input-container">
                        <input type="number" id="timeHours" class="time-input" min="0" max="23" placeholder="HH" value="00">
                        <span class="time-separator">:</span>
                        <input type="number" id="timeMinutes" class="time-input" min="0" max="59" placeholder="MM" value="00">
                    </div>
                ...
                return `
                    <div class="iso-input-container">
                        ...
                        <div class="iso-inputs">
                            ...
                            <span class="iso-separator">:</span>
                            <input type="number" id="isoDays" class="iso-input-wide" min="0" max="999" placeholder="DD">
                            <span class="iso-separator">:</span>
                        ...
                        <div class="iso-preview" id="isoPreview">P1Y2M3DT4H5M</div>
                    </div>
                `;
            default:
    ...
    clearActiveComponent(component, componentType) {
        ...
        if (componentType === 'calendar') {
            // Calendar komponentas naudoja clearSelection()
            if (component.clearSelection && typeof component.clearSelection === 'function') {
                component.clearSelection();
                console.log('Calendar cleared using clearSelection()');
            }
    ...
    updateGlobalPreview() {
        ...
        if (command) {
            outputEl.innerHTML = `<code>task add "${this.getTaskDescription()}" ${command}</code>`;
    ...
    generateFieldCommand(field, fieldData) {
        ...
        if (fieldData.date && fieldData.date.formatted) {
            commandValue = fieldData.date.formatted;
            if (fieldData.time && fieldData.time.formatted && fieldData.time.formatted !== '00:00') {
                commandValue += `T${fieldData.time.formatted}`;
            }
        }
        else if (fieldData.weekday && fieldData.weekday.formatted) {
            commandValue = fieldData.weekday.formatted;
        }
        else if (fieldData.month && fieldData.month.formatted) {
            commandValue = fieldData.month.formatted;
        }
        else if (fieldData.monthday && fieldData.monthday.formatted) {
            commandValue = fieldData.monthday.formatted;
        }
        else if (fieldData.interval && fieldData.interval.formatted) {
            commandValue = fieldData.interval.formatted;
        }
        else if (fieldData.intervalsIso && fieldData.intervalsIso.formatted) {
            commandValue = fieldData.intervalsIso.formatted;
        }
